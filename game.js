...
let cameraOrbiting=false, dashKeyLatch=false, moveVelX=0, moveVelZ=0;
const W=3200,H=W/2;
const zones=[
...
// ===== WORLD BUILD =====
function terrain(){const seg=220,size=W;const geo=new THREE.PlaneGeometry(size,size,seg,seg);geo.rotateX(-Math.PI/2);const pos=geo.attributes.position;const colors=[];for(let i=0;i<pos.count;i++){const x=pos.getX(i),z=pos.getZ(i),q=zoneAt(x,z);pos.setY(i,groundHeight(x,z));const c=new THREE.Color(q.color);c.offsetHSL((Math.sin(x*.01+z*.013)*.015),0,.03);colors.push(c.r,c.g,c.b)}geo.setAttribute('color',new THREE.Float32BufferAttribute(colors,3));const m=new THREE.MeshStandardMaterial({vertexColors:true,roughness:.92,metalness:0});const g=new THREE.Mesh(geo,m);g.receiveShadow=true;scene.add(g);worldGroup.add(g)}
...
function buildWorld(){worldGroup=new THREE.Group();scene.add(worldGroup);terrain();water();city();regionProps();volcano();portal(-850,650,0x55eaff,'PRADARIA');portal(900,700,0xffcc66,'DESERTO');portal(900,-750,0xd9f5ff,'GELEIRA');portal(-850,-700,0x58ff9a,'SELVA');portal(0,-250,0xff4a24,'VULCÃO');portal(0,700,0x8c9cff,'CIDADE');portal(0,-1040,0x55eaff,'LITORAL');
 ...
 for(let i=0;i<55;i++){const q=zones[Math.floor(Math.random()*5)];const a=Math.random()*6.28,r=80+Math.random()*550;collectible(q.x+Math.cos(a)*r,q.z+Math.sin(a)*r,i%3?'gem':'ring')}for(let i=0;i<18;i++){const q=zones[Math.floor(Math.random()*5)];const a=Math.random()*6.28,r=120+Math.random()*450;enemy(q.x+Math.cos(a)*r,q.z+Math.sin(a)*r,i%4)}
...
function init(){scene=new THREE.Scene();scene.background=new THREE.Color(0x7bb9e6);scene.fog=new THREE.FogExp2(0x7bb9e6,.00065);clock=new THREE.Clock();camera=new THREE.PerspectiveCamera(68,innerWidth/innerHeight,.1,5000);renderer=new THREE.WebGLRenderer({antialias:true,powerPreference:'high-performance',precision:'highp'});renderer.setSize(innerWidth,innerHeight);renderer.setPixelRatio(Math.min(devicePixelRatio,2.5));renderer.shadowMap.enabled=true;renderer.shadowMap.type=THREE.PCFSoftShadowMap;renderer.outputColorSpace=THREE.SRGBColorSpace;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=1.18;renderer.physicallyCorrectLights=true;document.body.appendChild(renderer.domElement);scene.add(remoteGroup);
 const hemi=new THREE.HemisphereLight(0xcceeff,0x23301f,1.55);scene.add(hemi);const sun=new THREE.DirectionalLight(0xffffff,3.2);sun.position.set(-600,900,500);sun.castShadow=true;sun.shadow.mapSize.set(2048,2048);sun.shadow.bias=-0.00012;sun.shadow.normalBias=0.025;sun.shadow.camera.left=-1050;sun.shadow.camera.right=1050;sun.shadow.camera.top=1050;sun.shadow.camera.bottom=-1050;scene.add(sun);scene.userData.sun=sun;
 ...
 buildWorld();player=new THREE.Group();playerVisual=playerModel(0x29d8ff);player.add(playerVisual);playerVisual.traverse(o=>{if(o.isMesh){o.castShadow=true;o.receiveShadow=true}});tag(player,'VOCÊ','#66eaff');player.position.set(0,groundHeight(0,500),500);scene.add(player);camera.position.set(0,9,15);camYaw=player.rotation.y; $('missionText').textContent='Explore os 7 biomas e encontre 3 portais.'; $('help').innerHTML='<b>WASD</b> mover · <b>SHIFT</b> turbo · <b>ESPAÇO</b> pular · <b>Q/E</b> dash · <b>BOTÃO DIREITO</b> câmera · <b>RODINHA</b> zoom · <b>M</b> mapa';window.addEventListener('resize',resize);document.addEventListener('keydown',keydown);document.addEventListener('keyup',e=>keys[e.code]=false);window.addEventListener('blur',()=>{for(const k in keys)keys[k]=false;cameraOrbiting=false;if(document.pointerLockElement===renderer.domElement)document.exitPointerLock?.()});setupCameraControls();requestAnimationFrame(loop)}
...
function move(dt){let f=0,s=0;if(keys.KeyW)f++;if(keys.KeyS)f--;if(keys.KeyD)s++;if(keys.KeyA)s--;const len=Math.hypot(f,s)||1;f/=len;s/=len;let sp=keys.ShiftLeft||keys.ShiftRight?42:24;if(boost>0)sp=68;const dir=new THREE.Vector3(s,0,-f).applyAxisAngle(new THREE.Vector3(0,1,0),camYaw);if(f||s){moveVelX=THREE.MathUtils.lerp(moveVelX,dir.x*sp,1-Math.pow(.00001,dt));moveVelZ=THREE.MathUtils.lerp(moveVelZ,dir.z*sp,1-Math.pow(.00001,dt));yaw=Math.atan2(-dir.x,-dir.z);player.rotation.y=THREE.MathUtils.lerpAngle(player.rotation.y,yaw,.16)}else{moveVelX=THREE.MathUtils.lerp(moveVelX,0,1-Math.pow(.000001,dt));moveVelZ=THREE.MathUtils.lerp(moveVelZ,0,1-Math.pow(.000001,dt))}player.position.x+=moveVelX*dt;player.position.z+=moveVelZ*dtif(keys.Space&&onGround){velY=18;onGround=false}const dashPressed=(keys.KeyQ||keys.KeyE); if(dashPressed&&!dashKeyLatch&&dash<=0&&(f||s)){dash=0.35;boost=0.5;player.position.addScaledVector(dir,18)} dashKeyLatch=dashPressed;velY-=45*dt;player.position.y+=velY*dt;const floor=groundHeight(player.position.x,player.position.z);if(player.position.y<=floor){player.position.y=floor;velY=0;onGround=true}if(Math.abs(player.position.x)>1550)player.position.x=Math.sign(player.position.x)*1550;if(Math.abs(player.position.z)>1550)player.position.z=Math.sign(player.position.z)*1550;dash-=dt;boost-=dt;playerVisual.position.y=onGround?Math.sin(clock.elapsedTime*12)*.03:0;playerVisual.userData.aura.rotation.y+=dt*3;}
